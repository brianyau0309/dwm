...
enum { SchemeNorm, SchemeSel, SchemeSelW }; /* color schemes */
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			...
			drw_setscheme(drw, scheme[m == selmon ? SchemeSelW : SchemeNorm]);
...
solitary(Client *c)
{
	return ((nexttiled(c->mon->cl->clients, selmon) == c && !nexttiled(c->next, selmon))
